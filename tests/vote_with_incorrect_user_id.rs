...
async fn post_vote_with_incorrect_user_id_returns_422_unprocessable_entity(
    _: SqlitePoolOptions,
...
async fn post_vote_with_incorrect_user_id_is_json_err_response(
    _: SqlitePoolOptions,
    db_options: SqliteConnectOptions,
) {
    let client = get_asynchronous_api_client(STATIC_DIR, db_options).await;
...
async fn post_vote_with_incorrect_user_id_returns_expected_error(
    _: SqlitePoolOptions,
    db_options: SqliteConnectOptions,
) {
    let client = get_asynchronous_api_client(STATIC_DIR, db_options).await;
